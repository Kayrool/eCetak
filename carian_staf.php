...
if (isset($_GET['action']) && $_GET['action'] === 'search') {
    ...
    try {
        ...
        while ($row = $stmt->fetch()) {
            ...
            $nama = isset($row['nama']) ? to_utf8($row['nama']) : null;
            $bah_fak = isset($row['bah_fak']) ? to_utf8($row['bah_fak']) : null;
            $emel = isset($row['emel']) ? to_utf8($row['emel']) : null;
            $tok = $no !== '' ? make_token($no, $TOKEN_SECRET) : null;

            $rows[] = [
                'nama'    => $nama,
                'jawatan' => isset($row['jawatan']) ? to_utf8($row['jawatan']) : null,
                'bah_fak' => $bah_fak,
                'emel'    => $emel,
                'token'   => $tok,
            ];
    ...
    $(document).on('click', '.btnPilih', function(){
      ...
      $.ajax({
        ...
        success: function(res){
          ...
          form.append($('<input type="hidden" name="nama">').val(res.nama || ''));
          form.append($('<input type="hidden" name="bah_fak">').val(res.bah_fak || ''));
          form.append($('<input type="hidden" name="emel">').val(res.emel || ''));
          $('body').append(form);
          form.submit();
        },
        error: function(xhr){
